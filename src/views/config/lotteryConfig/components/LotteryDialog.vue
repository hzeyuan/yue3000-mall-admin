  ...
  <div id="LotteryDialog">
    <el-dialog title="抽奖活动详情" :visible.sync="dialogShow" width="40%">
            ...
            <div class="flex flex-wrap">
              ...
              <div class="prize-box bg-gray-100 h-24 m-1 w-24">
                <div class="add-prize" @click="onAddDialog">
                  <i
                    class="el-icon-circle-plus-outline text-2xl leading-none"
                  ></i>
                  <p class="p-0">添加奖品</p>
                </div>
              </div>
              <!--              奖品数据-->
              <div
                class="prize-box bg-gray-100 h-24 m-1 w-24 text-center"
              ...
              >
                ...
                <div class="prize-operate">
                  ...
                  <i
                    ...
                    @click="onDeletePrize(item.id, index)"
                  ></i>
                </div>
              </div>
            </div>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="formSubmit('add')" v-if="isEdge">
          添 加
        </el-button>
        <el-button type="primary" @click="formSubmit('update')" v-else>
          修 改
        </el-button>
      </span>
    </el-dialog>
    ...
    >
      <div>
        ...
        >
          ...
          <el-form-item label="奖品图片">
            <IconUpload v-model="prizeData.image_url"></IconUpload>
          </el-form-item>
          <el-form-item label="奖品类型">
            <el-select
              v-model="prizeData.prize_type"
              placeholder="请选择奖品类型"
              @change="handlePrizeTypeVary(1)"
            >
              <el-option
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" v-if="prizeData.id" @click="onUpdatePrize()">
          修 改
        </el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 点击修改奖品
      onUpdatePrize() {
        updateLotteryActivityPrize(this.prizeData.id, this.prizeData).then(
          (res) => {
      ...
      formSubmit(action) {
        this.$refs['ref-activity-from'].validate((valid) => {
          ...
          const data = _.pick(this.activityData, [
            'id',
            'name',
            'prize_ids',
            'rule',
          ...
          if (action === 'add') {
            addLotteryActivityList(data).then(() => {
              this.$message({
                type: 'success',
                message: '修改成功',
                duration: 1000,
            ...
            updateLotteryActivityList(this.activityData.id, data).then(() => {
              this.$message({
                type: 'success',
                message: '修改成功',
                duration: 1000,
