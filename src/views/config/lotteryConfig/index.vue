      ...
      <div class="body-box">
        ...
        <div class="" v-show="menuShow === 1" style="width: 100%">
          ...
          <div class="table-container">
            <el-table :data="LotteryActivityList"  v-loading="listLoading">
              ...
              <el-table-column label="活动描述" align="center" width="300">
                <template slot-scope="scope">
                  <el-popover trigger="hover" placement="top">
                    <h5>活动规则：</h5>
                    <p
                      v-for="(item, index) in funStringRuleTOArrayRule(
                        scope.row.rule
                      )"
                      :key="index"
                    >
              ...
              <el-table-column label="活动开始时间" align="center" width="250">
                <template slot-scope="scope">2021-10-12 12:00</template>
              </el-table-column>
              <el-table-column label="活动结束时间" align="center" width="250">
                <template slot-scope="scope">2021-12-12 12:00</template>
              </el-table-column>
          ...
          <div class="page">
            <el-pagination
              ...
              :page-size="10"
              @prev-click="getLotteryActivityList"
              hide-on-single-page
              @next-click="getLotteryActivityList"
              @current-change="getLotteryActivityList"
            ></el-pagination>
          </div>
        </div>
        <div class="" v-show="menuShow === 2" style="width: 100%">
                ...
                <template slot-scope="scope">
                  <div class="info-box">
                    <div class="right">
                      ...
                      <p>
                        奖品类型：{{ scope.row.prize.prize_type | prizeType }}
                      </p>
                      <p v-if="scope.row.prize.prize_type == 1">
                        积分数量：{{ scope.row.prize.value }}
                      </p>
                      <p v-if="scope.row.prize.prize_type == 2">
                        优惠卷名称：{{ scope.row.prize.value }}
                      </p>
                    </div>
                  </div>
                </template>
          ...
          <div class="page">
            <el-pagination
              ...
              :page-size="10"
              @prev-click="getLotteryRecordList"
              hide-on-single-page
              @next-click="getLotteryRecordList"
              @current-change="getLotteryRecordList"
            ></el-pagination>
          </div>
        </div>
        <div class="" v-show="menuShow === 3" style="width: 100%">
          <div class="table-box">
            <el-table :data="LotteryRecordList">
              <el-table-column label="序号" align="center" width="100">
                <template slot-scope="scope">
    ...
    methods: {
      ...
      onDeleteDialog(id) {
        this.$confirm('是否要删除该活动', '提示', {
          ...
          deleteLotteryActivityList(id).then(async () => {
            this.$message({
              type: 'success',
              message: '删除成功',
              duration: 1000,
            })
            await this.getLotteryActivityList(1)
          })
        })
      },
      // 打开添加对话框
