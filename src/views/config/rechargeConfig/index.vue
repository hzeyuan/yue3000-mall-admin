  ...
  <el-card>
    ...
    <div class="body-box">
      <div class="menu-title">
        <div class="menu-item" :class='{show: menuShow === 1}' @click="menuShow = 1">充值方案</div>
        <div class="menu-item" :class='{show: menuShow === 2}' @click="menuShow = 2">充值设置</div>
      </div>
      <div class="sign-site" v-show="menuShow === 1">
        <div class="award-add">
          <el-button size="small" type="primary" @click="onAddDialog">新增充值规则</el-button>
        </div>
        <div class="award-table">
          <el-table :data="tableData" border style="width: 100%">
            ...
            <el-table-column label="推荐" align="center">
              <template slot-scope="scope">
                <el-switch :value="Boolean(scope.row.is_recommend)"></el-switch>
              </template>
            </el-table-column>
...
<!--  充值规则对话框-->
  <el-dialog title="充值方案" :visible.sync="dialogShow1" width="40%">
    <div style="width: 450px;margin: 0 auto">
      <el-form ref="ref-form" :model="form" label-width="150px">
        ...
        <el-form-item label="赠送金额：">
          <el-input v-model="form.give_money"
                    oninput="value=Number(value.replace(/[^0-9.]/g,''))"></el-input>
        </el-form-item>
        <el-form-item label="推荐：">
          <el-switch :value="form.is_recommend"></el-switch>
        </el-form-item>
    ...
    <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="onAddDate()" v-if="!form.id">添 加</el-button>
...
<script>
import { getConfigData, updateConfigData, getRechargeList, updateRechargeList } from "@/api/config";
...
export default {
  ...
  methods: {
    async reqGetData () {
      ...
      const res = await Promise.all([give_integral, give_growth, min_money, tableData])
    ...
    // 添加表格项参数
    reqAddTableData(){
      updateRechargeList(this.form).then((response) => {
        this.$message({
          type: "success",
          message: "添加成功",
          duration: 1000,
    ...
    // 删除表格充值规则
    reqDeleteTableData(){
      updateRechargeList(this.signRule).then((response) => {
        this.$message({
          type: "success",
          message: "删除成功",
          duration: 1000,
