    ...
    >
      ...
      <div slot="add">
        ...
        >
          ...
          <el-form-item
            :label="couponFormData.discount_type === 0 ? '折扣' : '金额'"
            prop="discount"
          >
            <el-input-number
              v-if="couponFormData.discount_type === 1"
              v-model="couponFormData.discount"
              :precision="2"
              :controls="false"
            ></el-input-number>
            <el-input-number
        ...
        <div style="text-align: right">
          <span class="dialog-footer">
            <el-button @click="$refs['strapi-table'].modal.visible = false">
              取 消
            </el-button>
            <el-button
              :loading="loading"
              type="primary"
              @click="addCouponData('coupon-form')"
            >
              确 定
            </el-button>
          </span>
    ...
    methods: {
      ...
      addCouponData(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          ...
          updateCoupon(data)
            .then((res) => {
              if (res.message === 'date.invalid') {
                this.$message({
                  message: '无效日期!',
                  type: 'warning',
                })
                this.couponFormData.time = ''
              } else if (res.message === 'code.invalid') {
                this.$message({
                  message: '优惠码格式不正确!',
                  type: 'warning',
                })
                this.couponFormData.code = ''
              } else if (res.message === 'code.exist') {
                this.$message({
                  message: '优惠码重复!',
                  type: 'warning',
                })
      ...
      handleChange(value) {
        if (value === 1) this.couponFormData.code = ''
      },
    },
  }
</script>
