<template>
  <div id="operateDialog">
    <el-dialog title="售后详情" :visible.sync="dialogShow" width="40%">
      <div class="max-h-96 overflow-y-auto">
        <el-divider content-position="left">售后信息</el-divider>
        <div class="flex flex-wrap p-5">
          <div class="w-1/3 mb-2">
            售后单号：{{ afterSaleData.after_sales.sn }}
          </div>
          <div class="w-1/3 mb-2">
            退款方式：{{ afterSaleData.after_sales.refund_type }}
          </div>
          <div class="w-1/3 mb-2">
            退款金额：{{ afterSaleData.after_sales.refund_price }}
          </div>
          <div class="w-1/3 mb-2">
            申请时间：{{ afterSaleData.created_at | formatCreateTime }}
          </div>
          <div class="w-1/3 mb-2">
            退款原因：{{ afterSaleData.after_sales.refund_reason }}
          </div>
          <div class="w-1/3 mb-2">
            退款说明：{{ afterSaleData.after_sales.refund_remark }}
          </div>
          <div class="w-1/3 mb-2">
            申请状态：{{ afterSaleData.after_sales.status | formatAfStatus }}
          </div>
          <div class="w-1/3 mb-2">
            售后说明：{{ afterSaleData.after_sales.refund_type }}
          </div>
        </div>
        <el-divider content-position="left">订单信息</el-divider>
        <div class="flex flex-wrap p-5">
          <div class="w-1/3 mb-2">
            订单编号：{{ afterSaleData.order.order_sn }}
          </div>
          <div class="w-1/3 mb-2">
            订单金额：{{ afterSaleData.order.order_price }}
          </div>
          <div class="w-1/3 mb-2">
            付款方式：{{ afterSaleData.order.payWay }}
          </div>
          <div class="w-1/3 mb-2">配送方式：物流配送</div>
          <div class="w-1/3 mb-2">
            订单状态：{{ afterSaleData.order.status | formatOrderStatus }}
          </div>
        </div>
        ...
        <div class="flex flex-wrap p-5">
          ...
          <div class="w-1/3 mb-2">
            会员昵称：{{ afterSaleData.user.username }}
          </div>
        ...
        <div class="pb-2">
          <el-table :data="afterSaleData.orderGoods" style="width: 100%">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button
          v-if="afterSaleData.after_sales.status == 0"
          type="success"
          @click="onSuccessBtn(afterSaleData.after_sales.status)"
        >
          审核申请退款
        </el-button>
        <el-button
          v-if="afterSaleData.after_sales.status == 0"
          type="warning"
          @click="onWarningBtn(afterSaleData.after_sales.status)"
        >
          商家拒绝
        </el-button>
        <el-button
          v-if="afterSaleData.after_sales.status == 2"
          type="success"
          @click="onSuccessBtn(afterSaleData.after_sales.status)"
        >
          商品待退货
        ...
        <el-button
          v-if="afterSaleData.after_sales.status == 3"
          type="success"
          @click="onSuccessBtn(afterSaleData.after_sales.status)"
        >
          商家待收货
        ...
        <el-button
          v-if="afterSaleData.after_sales.status == 5"
          type="success"
          @click="onSuccessBtn(afterSaleData.after_sales.status)"
        >
          等待退款
        ...
        <el-button
          v-if="afterSaleData.after_sales.status == 6"
          type="success"
          @click="onSuccessBtn(afterSaleData.after_sales.status)"
        >
          售后完成
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="innerVisibleShow = false">
          取 消
        </el-button>
