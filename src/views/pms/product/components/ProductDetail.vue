<template> 
  <el-card class="form-container" shadow="never">
    ...
    <el-form :model="goods" :rules="goodsRules" ref="goodsForm" label-width="120px" style="width: 600px" size="small">
      ...
      <el-form-item label="商品图片：">
        <picture-upload :isEdit="isEdit" :gallery="goods.gallery"></picture-upload>
      </el-form-item>
      ...
      <el-form-item label="简单描述：">
        <el-input v-model="goods.brief" type="textarea" :autoSize="true"></el-input>
      </el-form-item>
      ...
      <el-form-item label="规格"
                    v-show="!productsShow">
        <InputButton span="开放商品规格" @click.native="addSpecification"></InputButton>
      </el-form-item>
      <!--      商品规格项-->
      <el-form-item :label="'规格' + (index + 1) "
        ...
        <InputTable :options-data="item">
          <i class="el-icon-close" @click.prevent="deleteSpecification(index)"></i>
        </InputTable>
      </el-form-item>
      <!--      增加商品规格-->
      <el-form-item label=""
                    v-show="productsShow">
        <InputButton span="增加商品规格" @click.native="addSpecification"></InputButton>
      </el-form-item>
      <!--      商品规格表-->
      <el-form-item label="规格表"
                    v-if="productsShow">
        <OptionsTable :options-table="goods.goods_products" :options-title="goods.goods_specifications"></OptionsTable>
      </el-form-item>
...
<script>
  ...
  // 处理商品规格的工具
  import {getSpecifications, setSpecifications, arrMultiply, multiply} from '@/utils/multiply'
  ...
  export default {
    ...
    watch: {
      ...
      'goods.goods_specifications':{
        ...
        handler() {
          let arrobj = []
          let a = this.goods.goods_specifications
          if (a.length === 1){
            this.goods.goods_specifications[0].value.forEach((item)=>{
              arrobj.push({
                specifications:[item],
                number: '',
                price: '',
        ...
        this.$refs['goodsForm'].validate((valid) => {
          if (valid) {
            this.$confirm('是否要提交该产品', '提示', {
              ...
              if(isEdit){
                updateGoods(this.$route.query.id,this.productParam).then(response=>{
                  this.$message({
                    type: 'success',
                    message: '修改成功',
                    duration:1000
                ...
                data.goods_specifications = getSpecifications(this.goods.goods_specifications)
