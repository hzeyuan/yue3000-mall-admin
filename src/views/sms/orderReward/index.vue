  ...
  <el-card>
    ...
    <div class="form-box">
      <el-form ref="form" :model="form" label-width="150px">
        <el-form-item label="下单奖励积分：">
          <el-input v-model.number="form.order_award_integral"
...
export default {
  ...
  methods: {
    // 获取赠送积分参数
    async reqGetData () {
      const res = await getConfigData('MarketingConfig','orderAwardConfig')
      this.form.order_award_integral = res.value
      console.log(res)
    },
    // 修改积分参数
    reqUpdateData (data) {
      updateConfigData('MarketingConfig', 'orderAwardConfig', data).then((response) => {
        this.$message({
          type: "success",
          message: "修改成功",
          duration: 1000,
