  ...
  <div id="signDaily">
      ...
      <div class="body-box">
        ...
        <div class="sign-site" v-show="menuShow === 1">
          <div class="award-config">
            <div class="text">每天签到奖励：</div>
            <el-form
              ref="signRule"
              :model="signRule.dailySign"
              label-width="150px"
            >
              <el-form-item label="签到送积分：">
                <el-input
                  v-model.number="signRule.dailySign.integral"
                  oninput="value=Number(value.replace(/[^0-9.]/g,''))"
                ></el-input>
              </el-form-item>
              <el-form-item label="签到送成长值：">
                <el-input
                  v-model.number="signRule.dailySign.growth"
                  oninput="value=Number(value.replace(/[^0-9.]/g,''))"
                ></el-input>
              </el-form-item>
            </el-form>
          ...
          <div class="award-config">
            <div class="text">连续签到奖励：</div>
            <p class="span">
              连续签到奖励断签后会重新计算连续签到天数，达到连续天数后即可获得连续奖励
            </p>
          </div>
          <div class="award-add">
            ...
            <el-table :data="tableData" border style="width: 100%">
              ...
              <el-table-column label="连续奖励" align="center">
                <template slot-scope="scope">
                  {{ scope.row.integral | integral
                  }}{{ scope.row.growth | growth }}
                </template>
              </el-table-column>
          ...
          <div class="award-config" style="margin-top: 10px">
            ...
            <div style="margin-left: 20px">
              <el-button size="small" @click="onDefaultButton">
                使用默认说明
              </el-button>
            </div>
          </div>
          <div class="award-button">
            <el-button size="small" type="primary" @click="onButton">
              确认修改
            </el-button>
          </div>
        </div>
        <div class="sign-record" v-show="menuShow === 2">
          <div class="filter">
            <div class="filter-title">会员信息筛选</div>
            ...
            <div class="filter-button">
              <el-button type="primary" @click="onSearchButton">
                查 询
              </el-button>
          ...
          <div class="table">
            <el-table :data="recordData" border style="width: 100%">
              ...
              <el-table-column label="连续奖励" align="center" width="250">
                <template slot-scope="scope">
                  {{ scope.row.continuous_integral | integral }}
                  {{ scope.row.continuous_growth | growth }}
                </template>
              </el-table-column>
          ...
          <div class="page">
            <el-pagination
              ...
              :page-size="10"
              @prev-click="handlePageTo"
              hide-on-single-page
              @next-click="handlePageTo"
              @current-change="handlePageTo"
            ></el-pagination>
          </div>
        </div>
      </div>
    ...
    <el-dialog title="连续签到规则" :visible.sync="dialogShow" width="40%">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="onAddDate()" v-if="isEdge">
          添 加
        </el-button>
        <el-button type="primary" @click="onUpdateDate()" v-else>
          修 改
        </el-button>
      </span>
    </el-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      async reqGetRecordData() {
        const res = await getSignRecordData(
