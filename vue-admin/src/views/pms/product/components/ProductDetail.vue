<template>
  ...
  <el-card class="form-container" shadow="never">
    ...
    >
      ...
      <el-form-item
        :label="'规格' + (index + 1)"
        v-for="(item, index) in goods.specifications"
        :key="index"
      >
      ...
      <!--      增加商品规格-->
      <el-form-item label="" v-show="goods.specifications.length === 1">
        <InputButton
          span="增加商品规格"
          @click.native="addSpecification"
        ></InputButton>
      ...
      <el-form-item label="规格表" v-if="productsShow">
        <OptionsTable
          ...
          :options-title="goods.specifications"
        ></OptionsTable>
      </el-form-item>
...
export default {
  ...
  watch: {
    ...
    'goods.specifications': {
      deep: true,
      handler() {
        const { specifications } = this.goods
        if (_.isEmpty(specifications)) return
        const primarySpec = specifications[0] // 第一个 规格 [{key:'',value:}]
        const secondSpec = specifications[1] // 第二个 规格
        // 如
        ...
        for (let i = 0; i < arrobj.length; i++) {
          this.goods.goods_products.forEach((item) => {
            if (_.isEqual(item.specifications, arrobj[i].specifications)) {
              arrobj[i] = _.cloneDeep(item)
            }
          })
        }
  ...
  methods: {
    ...
    async getGoods(id) {
      ...
      const { specifications_products, is_new, is_hot, is_on_sale } = data
      let specifications = []
      for (let spec in data.specifications) {
        specifications.push({
          ...
          pic_url: '',
          id: data.specifications[spec].join('/')
        })
      }
      // {
      //   specifications:['红','小'],  //规格选项名
      ...
      this.goods = {
        ...
        goods_products: Object.values(specifications_products).map((data) => ({
          ...data,
      ...
      this.$refs['goodsForm'].validate((isValid) => {
        ...
        this.$confirm('是否要提交该产品', '提示', {
          ...
          if (isEdit) {
            ...
            data.specifications = getSpecifications(data.specifications)
            data.category_id = data.category_id.slice(-1)[0];
            // 修改商品
            updateGoods(this.$route.query.id, data).then(
              (response) => {
                this.$message({
                  type: 'success',
                  message: '修改成功',
